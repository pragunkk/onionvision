...
import { toPng } from 'html-to-image'; // <--- Replaced html2canvas
...
export default function BatchDashboard() {
  ...
  // Diameter buckets: <35 (Reject), 35-50 (Med), 50-70 (Large), >70 (Reject)
  const diameterBuckets = { '<35mm': 0, '35-50mm': 0, '50-70mm': 0, '>70mm': 0 };

  currentBatch.forEach(onion => {
    // Grades
  ...
  const pieData = Object.entries(defectCounts)
  ...
  const handleGenerateReceipt = async () => {
    ...
    // Temporarily apply solid styling for the PDF render
    receiptRef.current.style.backgroundColor = '#ffffff';
    receiptRef.current.style.color = '#000000';
    
    try {
      // Use toPng which natively supports oklch and SVGs
      ...
      // Calculate correct aspect ratio height using jsPDF's image property reader
      ...
      // Finalize batch and push to DB
      await finishBatch();
    } catch (err) {
      console.error("PDF Generation failed:", err);
      // Revert styles just in case it fails
      receiptRef.current.style.backgroundColor = '';
      receiptRef.current.style.color = '';
    }
  };
      ...
      <div ref={receiptRef} className="p-6 bg-neutral-950 border border-neutral-800 rounded-xl space-y-8">
        ...
        <div className="flex justify-between items-end border-b border-neutral-800 pb-4">
          ...
          <div className="text-right">
            ...
            <p className="text-3xl font-bold text-neutral-100">{currentBatch.length}</p>
          </div>
        </div>
